...
/** This stream applies a `project` function to events fired by its parent and
  * fires the resulting value
  *
  * This stream emits an error if the parent observable emits an error or if
  * `project` throws
  *
  ...
  *   - Not handle the error (meaning .isDefinedAt(error) must be false) to emit
  *     the original error
  *
  * If `recover` throws an exception, it will be wrapped in `ErrorHandlingError`
  * and propagated.
  *
  * @param project
    ...
    recover.fold(
      ...
      Try(pf.applyOrElse(nextError, (_: Throwable) => null)).fold(
        tryError => {
          ...
          fireError(
            ErrorHandlingError(error = tryError, cause = nextError),
            transaction
          )
        },
        nextValue => {
