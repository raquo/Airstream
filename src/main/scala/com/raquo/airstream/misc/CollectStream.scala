...
/** This stream applies `fn` to the parent stream's events, and emits `x` from
  * the resulting `Some(x)` value (if `None`, nothing is fired).
  *
  * This stream emits an error if the parent stream emits an error (Note: no
  * filtering applied), or if `fn` throws
  *
  * @param fn
...
class CollectStream[A, B](
    ...
    fn: A => Option[B]
) extends SingleParentStream[A, B]
