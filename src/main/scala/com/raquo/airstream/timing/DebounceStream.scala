...
/** This stream emits the last event emitted by `parent`, but only after
  * `intervalMs` has elapsed since `parent` emitted the previous event.
  *
  * Essentially, this stream emits the parent's last event, but only once the
  * parent stops emitting events for `intervalMs`.
  *
...
class DebounceStream[A](
  ...
  /** Every time [[parent]] emits an event, we clear the previous timer and set
    * a new one. This stream only emits when the parent has stopped emitting for
    * [[intervalMs]] ms.
    */
