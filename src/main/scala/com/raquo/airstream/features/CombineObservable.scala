...
trait CombineObservable[A] extends SyncObservable[A] { self =>
  ...
  // Implementations should call internalObserver.onNext() instead of .fireValue()
  // Transaction will call .fireSync() when it's time, and that will in turn call .fireValue()
