...
trait EventStream[+A] extends Observable[A] {
  ...
  override protected[this] def fireValue(nextValue: Any, transaction: Transaction): Unit = {
    ...
    externalObservers[A].foreach { observer =>
      try {
        observer.onNext(nextValue.asInstanceOf[A])
      } catch {
    ...
    internalObservers[A].foreach { observer =>
      observer.onNext(nextValue.asInstanceOf[A], transaction)
    }
  }
  ...
  override protected[this] final def fireTry(nextValue: Try[Any], transaction: Transaction): Unit = {
    nextValue.fold(
