...
trait EventStream[+A]
  ...
  /** Apply `pf` to each event and emit the resulting value, or emit nothing if
    * `pf` is not defined for that event.
    *
    * @param pf
    *   Note: guarded against exceptions
    */
  def collect[B](pf: PartialFunction[A, B]): EventStream[B] = collectOpt(
    pf.lift
  )

  /** Apply `fn` to parent stream event, and emit resulting x if it returns
    * Some(x)
    *
    * @param fn
    *   Note: guarded against exceptions
  ...
  /** Based on [[delay]], but tracks the status of input and output to that
    * operator. See [[Status]].
    */
  ...
  /** Make a stream that emits this stream's values but waits for `after` stream
    * to emit first in a given transaction. You can use this for Signals too
    * with `Signal.composeChanges` (see docs for more details)
    */
  ...
  /** Based on [[throttle]], but tracks the status of input and output to that
    * operator. See [[Status]].
    */
  def throttleWithStatus(
      ms: Int,
      leading: Boolean = true
  ): EventStream[Status[A, A]] = {
  ...
  /** Based on [[debounce]], but tracks the status of input and output to that
    * operator. See [[Status]].
    */
  ...
  /** Drop (skip) the first `numEvents` events from this stream. Note: errors
    * are NOT dropped.
    *
    * @param resetOnStop
  ...
  /** Drop (skip) events from this stream as long as they pass the test (as soon
    * as they stop passing, stop dropping) Note: errors are NOT dropped.
    *
    * @param passes
    *   Note: MUST NOT THROW!
    * @param resetOnStop
  ...
  /** Drop (skip) events from this stream as long as they do NOT pass the test
    * (as soon as they start passing, stop dropping) Note: errors are NOT
    * dropped.
    *
    * @param passes
    *   Note: MUST NOT THROW!
  ...
  /** Take the first `numEvents` events from this stream, ignore the rest. Note:
    * As long as events are being taken, ALL errors are also taken
    *
    * @param resetOnStop
    *   Reset the count if the stream stops
    */
  ...
  /** Imitate parent stream as long as events pass the test; stop emitting after
    * that.
    *
    * @param passes
    *   Note: MUST NOT THROW!
  ...
  /** Imitate parent stream as long as events to NOT pass the test; stop
    * emitting after that.
    *
    * @param passes
    *   Note: MUST NOT THROW!
  ...
  /** Returns a stream that emits events from this stream AND all off the
    * `streams`, interleaved. Note: For other types of combination, see
  ...
  def foldLeftRecover[B](initial: Try[B])(
      fn: (Try[B], Try[A]) => Try[B]
  ): Signal[B] = scanLeftRecover(initial)(fn)
  ...
  /** Convert stream to signal, given an initial value
    ...
    *   if false, signal's initial value will be re-evaluated on every restart
    *   (so long as the parent stream does not emit any values)
    */
  @inline def startWith[B >: A](
      initial: => B,
  ...
  /** @param cacheInitialValue
    ...
    *   (so long as the parent stream does not emit any values)
    */
  def toSignalWithEither[B >: A](
      initial: => Either[Throwable, B],
      cacheInitialValue: Boolean = false
  ): Signal[B] = {
  ...
  /** See also various debug methods in
...
object EventStream {
  ...
  /** @param emitOnce
    *   if true, the event will be emitted at most one time. If false, the event
    *   will be emitted every time the stream is started.
    */
  ...
  def fromFuture[A](future: Future[A], emitOnce: Boolean = false)(implicit
      ec: ExecutionContext
  ): EventStream[A] = {
  ...
  /** Create a stream from a [[java.util.concurrent.Flow.Publisher]]
    *   - Use this to bring in events from other streaming libraries that can
    *     provide a `Flow.Publisher`, such as FS2 an Monix.
    */
  def fromPublisher[A](
  ...
  /** Easy helper for custom events. See [[CustomStreamSource]] for docs.
    ...
    * stop.
    *
    * The arguments to `start` are functions. Call them to do things like emit
    * an event, emit an error, or get some info:
    *
  ...
  /** Create a stream of Unit, and a callback that, when fired, makes that
    * stream emit.
    */
  ...
  /** Create a stream and a JS callback that, when fired, makes that stream
    * emit.
    */
  ...
  /** Create a stream and an observer that, when receiving an event or an error,
    * makes that stream emit.
    */
  ...
  @inline def combineSeq[A](streams: Seq[EventStream[A]]): EventStream[Seq[A]] =
    sequence(streams)
  ...
  implicit def toEventStreamCompanionCombineSyntax(
      @unused s: EventStream.type
  ): StaticStreamCombineOps.type = StaticStreamCombineOps

  /** Provides methods on EventStream: combine, combineWith, withCurrentValueOf,
    * sample
    */
  implicit def toCombinableStream[A](
      stream: EventStream[A]
  ): CombinableStream[A] = new CombinableStream(stream)

  /** Provides methods on EventStream: split, splitByIndex */
  implicit def toSplittableStream[M[_], Input](
      stream: EventStream[M[Input]]
  ): SplittableStream[M, Input] = new SplittableStream(stream)

  /** Provides methods on EventStream: splitOne */
  implicit def toSplittableOneStream[A](
      stream: EventStream[A]
  ): SplittableOneStream[A] = new SplittableOneStream(stream)

  /** Provides methods on stream: splitBoolean */
  implicit def toBooleanStream[A](stream: EventStream[Boolean]): BooleanStream =
    new BooleanStream(stream)

  /** Provides methods on stream: filterSome, collectSome, splitOption */
  implicit def toOptionStream[A](
      stream: EventStream[Option[A]]
  ): OptionStream[A] = new OptionStream(stream)

  /** Provides methods on stream: collectLeft, collectRight */
  implicit def toEitherStream[A, B](
      stream: EventStream[Either[A, B]]
  ): EitherStream[A, B] = new EitherStream(stream)

  /** Provides methods on stream: collectFailure, collectSuccess */
  implicit def toTryStream[A](stream: EventStream[Try[A]]): TryStream[A] =
    new TryStream(stream)

  /** Provides methods on stream: collectOutput, collectResolved,
    * collectPending, collectPendingInput, splitStatus
    */
  implicit def toStatusStream[In, Out](
      stream: EventStream[Status[In, Out]]
  ): StatusStream[In, Out] = new StatusStream(stream)

  /** Provides debug* methods on EventStream: debugSpy, debugLogEvents,
    * debugBreakErrors, etc.
    */
  implicit def toDebuggableStream[A](
      stream: EventStream[A]
  ): DebuggableStream[A] = new DebuggableStream[A](stream)

  // toTupleStreamN conversions provide mapN and filterN methods on Signals of tuples

  implicit def toTupleStream2[T1, T2](
      stream: EventStream[(T1, T2)]
  ): TupleStream2[T1, T2] = new TupleStream2(stream)

  implicit def toTupleStream3[T1, T2, T3](
      stream: EventStream[(T1, T2, T3)]
  ): TupleStream3[T1, T2, T3] = new TupleStream3(stream)

  implicit def toTupleStream4[T1, T2, T3, T4](
      stream: EventStream[(T1, T2, T3, T4)]
  ): TupleStream4[T1, T2, T3, T4] = new TupleStream4(stream)

  implicit def toTupleStream5[T1, T2, T3, T4, T5](
      stream: EventStream[(T1, T2, T3, T4, T5)]
  ): TupleStream5[T1, T2, T3, T4, T5] = new TupleStream5(stream)

  implicit def toTupleStream6[T1, T2, T3, T4, T5, T6](
      stream: EventStream[(T1, T2, T3, T4, T5, T6)]
  ): TupleStream6[T1, T2, T3, T4, T5, T6] = new TupleStream6(stream)
