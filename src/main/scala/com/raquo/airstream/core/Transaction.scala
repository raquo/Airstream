...
/** Transaction is a moment in time during which Airstream guarantees no FRP
  * glitches.
  *
  * Some observables need to emit their events in new transactions. Roughly
  * speaking:
  *   - All async observables (e.g. delay(100))
  *   - All observables that can create loops in the observable graph (e.g.
  *     flatMapSwitch)
  *   - All observables that get their events from outside of Airstream (e.g.
  *     custom sources)
  *
...
class Transaction(private[Transaction] var code: Transaction => Any) {
  ...
  /** Priority queue of pending observables: sorted by their topoRank.
    *
    * Corollary: An Observable that is dequeue-d from here does not
    * synchronously depend on any other pending observables
    */
  private[this] var maybePendingObservables
      : js.UndefOr[JsPriorityQueue[SyncObservable[_]]] = js.undefined

  /** Note: The transaction may be _actually scheduled_ one layer deeper than
    * this `depth` due to the `postStartTransactions` mechanism, but this is
    * good enough for `maxDepth` purposes.
    */
  private val depth: Int =
    Transaction.pendingTransactions.peekStack().fold(1)(_.depth + 1)
  ...
  if (Transaction.maxDepth == -1 || depth > Transaction.maxDepth) {
    ...
    AirstreamError.sendUnhandledError(
      TransactionDepthExceeded(this, Transaction.maxDepth)
    )
  } else {
  ...
  private[airstream] def containsPendingObservable(
      observable: SyncObservable[_]
  ): Boolean = {
    maybePendingObservables.map(_.contains(observable)).getOrElse(false)
  }

  private[airstream] def enqueuePendingObservable(
      observable: SyncObservable[_]
  ): Unit = {
...
object Transaction {
  ...
  /** Create new transaction.
    ...
    * Example of legitimate use case:
    * [[https://github.com/raquo/Airstream/#var-transaction-delay Var transaction delay]]
    */
  ...
  /** How many _nested_ transactions you can have.
    ...
    *
    * You probably need to adjust your code instead of raising this limit, but
    * if you have a valid need for raising it, please let me know.
    *
    * You can set this to -1 to disable the check completely, but then if you do
    * actually have an infinite loop, your code will pin the CPU and your single
    * threaded Scala.js app will freeze – not something you want, especially on
    * your users' devices!
    */
  var maxDepth: Int = 1000

  /** This object holds a queue of callbacks that should be executed when all
    * observables finish starting. This lets `signal.changes` streams emit the
    * updated signal's value when restarting, in such a way that the value
    * propagates to all new observers instead of just the first new observer
    * that triggered restart.
    *
    ...
    * Airstream, and in a couple places in Laminar, and this seems to cover most
    * reasonable use cases. Users might need to wrap some of their code into
    * `onStart.shared` manually if they manage subscriptions manually.
    *
    * See
    * https://github.com/raquo/Airstream/#restarting-streams-that-depend-on-signals--signalchanges-
    */
  object onStart {
  ...
  private object pendingTransactions {
    ...
    /** First transaction is the top of the stack, currently running. That
      * transaction's parent transaction is the second item, and so on.
      */
    ...
    @tailrec def done(transaction: Transaction): Unit = {
      ...
      transaction.code =
        throwDeadTrxError // stop holding up `trx` contents in memory
      ...
      if (maybeNextTransaction == js.undefined) {
        if (children.size > 0) {
          ...
          var numChildren = 0
          children.forEach((transactions, _) =>
            numChildren += transactions.length
          )
          throw new Exception(
            s"Transaction queue error: Stack cleared, but a total of ${numChildren} children for ${children.size} transactions remain. This is a bug in Airstream."
          )
        }
      } else {
     ...
     */
    @tailrec def putNextTransactionOnStack(
      ...
      if (maybeNextChildTrx == js.undefined) {
        ...
        if (maybeParentTransaction != js.undefined) {
          val parentTransaction =
            maybeParentTransaction.asInstanceOf[Transaction]
    ...
    /** Returns the top of the stack, i.e. the currently executing transaction,
      * if any.
      */
    ...
    private def maybeChildrenFor(
        transaction: Transaction
    ): js.UndefOr[JsArray[Transaction]] = {
    ...
    private def enqueueChild(
        parent: Transaction,
        newChild: Transaction
    ): Unit = {
  ...
  private[airstream] def currentTransaction(): js.UndefOr[Transaction] =
    pendingTransactions.peekStack()
  ...
  private val throwDeadTrxError: Transaction => Any = { trx =>
    throw new Exception(
      s"Attempted to run Transaction $trx after it was already executed."
    )
  }
