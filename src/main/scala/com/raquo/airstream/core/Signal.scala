...
import com.raquo.airstream.misc.{MapEventStream, MapSignal}
...
trait Signal[+A] extends Observable[A] with BaseObservable[Signal, A] with SignalSource[A] with SignalOps[A] {
