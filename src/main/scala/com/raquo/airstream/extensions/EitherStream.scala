...
class EitherStream[A, B](val stream: EventStream[Either[A, B]]) extends AnyVal {
  ...
  /** Emit `pf(x)` if parent stream emits `Left(x)` and `pf` is defined for `x`,
  ...
  /** Emit `pf(x)` if parent stream emits `Right(x)` and `pf` is defined for
  ...
  /** This `.split`-s a stream of Either-s by their `isRight` property. If you
    * want a different key, use the .splitOne operator directly.
    *
    * @param left
    *   (initialLeft, signalOfLeftValues) => output `left` is called whenever
    *   `stream` switches from `Right()` to `Left()`. `signalOfLeftValues`
    *   starts with `initialLeft` value, and updates when the parent stream
    *   updates from `Left(a)` to Left(b)`.
    * @param right
    *   (initialRight, signalOfRightValues) => output `right` is called whenever
    *   `stream` switches from `Left()` to `Right()`. `signalOfRightValues`
    *   starts with `initialRight` value, and updates when the parent stream
    *   updates from `Right(a)` to `Right(b)`.
    */
  def splitEither[C](
      left: (A, Signal[A]) => C,
      right: (B, Signal[B]) => C
        ...
        case Right(v) =>
          right(
            ...
            signal.map(e =>
              e.getOrElse(
                throw new Exception(
                  s"splitEither: `${stream}` bad right value: $e"
                )
              )
            )
          )
        case Left(v) =>
          left(
            ...
            signal.map(e =>
              e.left.getOrElse(
                throw new Exception(
                  s"splitEither: `${stream}` bad left value: $e"
                )
              )
            )
          )
