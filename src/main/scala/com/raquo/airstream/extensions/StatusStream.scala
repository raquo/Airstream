...
class StatusStream[In, Out](val stream: EventStream[Status[In, Out]])
    extends AnyVal {
  ...
  /** Emit `pf(x)` if parent stream emits `Resolved(_, x, _)`, do nothing
    * otherwise
    */
  ...
  /** Emit `pf(Resolved(...))` if parent stream emits Resolved(...), do nothing
    * otherwise
    */
  ...
  /** Emit `pf(Resolved(...))` if parent stream emits Resolved(...) and `pf` is
    * defined for it, do nothing otherwise
    */
  def collectResolved[Out2](
      pf: PartialFunction[Resolved[In, Out], Out2]
  ): EventStream[Out2] = {
    stream.collectOpt {
  ...
  /** This `.split`-s a stream of Statuses by their type (resolved vs pending).
    ...
    *   whenever `stream` switches from `Pending` to `Resolved`.
    *   `signalOfResolvedValues` starts with `initialResolved` value, and
    *   updates when the parent stream emits a new `Resolved` consecutively
    *   after another `Resolved`.
    * @param pending
    *   (initialPending, signalOfPendingValues) => output `pending` is called
    *   whenever `stream` switches from `Resolved` to `Pending`, or when the
    *   very first event is emitted (and it's `Pending`, as is typical)
    *   `signalOfPendingValues` starts with `initialPending` value, and updates
    *   when the parent stream emits a new `Resolved` consecutively after
    *   another `Resolved`. This happens when the stream emits inputs faster
    *   than the outputs are resolved.
    */
  def splitStatus[A](
