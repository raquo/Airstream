...
class TryStream[A](val stream: EventStream[Try[A]]) extends AnyVal {
  ...
  /** Emit `pf(x)` if parent stream emits `Success(x)` and `pf` is defined for
  ...
  def collectFailure: EventStream[Throwable] = stream.collect {
    case Failure(ev) => ev
  }

  /** Emit `pf(x)` if parent stream emits `Failure(x)` and `pf` is defined for
    * `x`, do nothing otherwise
    */
  ...
  /** This `.split`-s a stream of Try-s by their `isSuccess` property. If you
    ...
    *   when the parent stream updates from `Failure(a)` to `Failure(b)`.
    */
  def splitTry[B](
