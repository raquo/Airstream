...
class StatusObservable[In, Out, Self[+_] <: Observable[_]](
  ...
  /** Map Resolved(...) to Right(project(Resolved(...))), and Pending(y) to
    * Left(Pending(y))
    */
  def mapResolved[B](
  ...
  /** Map Pending(x) to Left(project(Pending(x))), and Resolved(...) to
    * Right(Resolved(...))
    */
  def mapPending[B](
  ...
  /** Map Resolved(...) with `resolved`, and Pending(x) with `pending`, to a
    * common type
    */
  def foldStatus[B](
