...
class OptionStream[A](val stream: EventStream[Option[A]]) extends AnyVal {
  ...
  /** Emit `pf(x)` if parent stream emits `Some(x)` and `pf` is defined for `x`,
  ...
  /** This `.split`-s a Stream of an Option by the Option's `isDefined`
    ...
    *   starts with `initialInput` value, and updates when the parent stream
    *   updates from `Some(a)` to `Some(b)`.
    * @param ifEmpty
    *   \- returned if Option is empty, or if the parent stream has not emitted
