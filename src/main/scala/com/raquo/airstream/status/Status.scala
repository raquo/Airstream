...
/** Represents a combination of input event with the status (availability) of
  * output event(s) derived from it. The output events typically come from a
  * stream that is derived from the input stream, but emitting asynchronously,
...
case class Resolved[+In, +Out](input: In, output: Out, ix: Int)
  ...
  override def mapInput[In2](project: In => In2): Status[In2, Out] =
    copy(input = project(input))

  override def mapOutput[Out2](project: Out => Out2): Status[In, Out2] =
