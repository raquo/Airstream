...
/** #TODO[Naming,Org] Messy
  *
  * This signal offers the API of a [[StrictSignal]] but is actually lazy. All
  * it does is let you PULL the signal's current value. This mostly works fine
  * if your signal does not depend on any streams.
  *
  * We use this signal internally for derived Var use cases where we know that
  * it should work fine. we may change the naming and structure of this class
  * when we implement settle on a long term strategy for peekNow / pullNow
  * https://github.com/raquo/Laminar/issues/130
  */
class LazyStrictSignal[I, O](
...
) extends MapSignal[I, O](parentSignal, project = zoomIn, recover = None)
    with StrictSignal[O] { self =>
  ...
  override protected[state] def updateCurrentValueFromParent(
      nextValue: Try[O],
