...
/** EventBus exposes a Writer and a stream of its events.
  *
...
class EventBus[A] extends EventSource[A] with Sink[A] with Named {
  ...
  val writer: Writer[A] = new Writer[A]
...
object EventBus {
  ...
  def emit[A](
    ...
    Writer.emit(values.map(value => (value._1.writer, value._2)): _*)
  }
  ...
  def emitTry[A](
    ...
    Writer.emitTry(values.map(value => (value._1.writer, value._2)): _*)
  }
}
