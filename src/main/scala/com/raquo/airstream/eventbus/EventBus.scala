...
/** EventBus combines a WriteBus and a stream of its events.
  ...
  * should only have access to writing events, not reading all events from the
  * bus.
  */
...
object EventBus {
  ...
  implicit class EventBusTryTuple[A](val tuple: (EventBus[A], Try[A]))
      extends AnyVal
  ...
  /** Emit events into several WriteBus-es at once (in the same transaction)
    * Example usage: emitTry(eventBus1 -> Success(value1), eventBus2 ->
    * Failure(error2))
    */
  def emitTry(
  ...
  @inline private def toWriterTuple[A](
      t: EventBusTuple[A]
  ): WriteBus.BusTuple[A] =
    new WriteBus.BusTuple((t.tuple._1.writer, t.tuple._2))

  @inline private def toWriterTryTuple[A](
      t: EventBusTryTuple[A]
  ): WriteBus.BusTryTuple[A] =
