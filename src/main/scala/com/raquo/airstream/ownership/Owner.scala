...
/** Owner decides when to kill its subscriptions.
  ...
  *   - In other words: Owner can only own a Subscription once, and a
  *     Subscription can only ever be owned by its initial owner
  ...
  *
  * If you need something more flexible, use [[DynamicOwner]], or build your own
  * custom logic on top of this in a similar manner.
  */
trait Owner {

  /** Note: This is enforced to be a sorted set outside the type system.
    * #performance
    */
  ...
  @inline private[ownership] def _killSubscriptions(): Unit =
    killSubscriptions()

  /** This method will be called when this [[Owner]] has just started owning
    * this resource. You can override it to add custom behaviour. Note: You can
    * rely on this base method being empty.
    */
  protected[this] def onOwned(@unused subscription: Subscription): Unit = ()

  private[ownership] def onKilledExternally(
    ...
    if (index != -1) {
      ...
      throw new Exception(
        "Can not remove Subscription from Owner: subscription not found."
      )
    }
  }
