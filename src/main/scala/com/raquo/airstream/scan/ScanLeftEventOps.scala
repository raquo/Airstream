...
trait ScanLeftEventOps[+A] extends ScanLeftOps[Signal, EventStream, A] {
  ...
  @inline final def reduceLeft[B >: A](fn: (Try[B], Try[A]) => Try[B]): EventStream[B] = {
    reduceLeftOptionRecover(fn).updates.collect { case Some(value) => value }
  }
