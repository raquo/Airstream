...
/** Base functionality for a custom observable based on start and stop
  * callbacks.
  *
  * See:
...
trait CustomSource[A] extends WritableObservable[A] {
  ...
  /** CustomSource is intended for observables that don't synchronously depend
  ...
  override protected[this] def onStart(): Unit = {
    Try(config.onStart()).recover[Unit] { case err: Throwable =>
      Transaction(fireError(err, _))
    }
    super.onStart()
  }
...
object CustomSource {
  ...
  final class Config private (
    ...
    /** Create a version of a config that only runs start / stop if the
      * predicate passes.
      *   - `start` will be run when the CustomSource is about to start if
      *     `passes` returns true at that time
      *   - `stop` will be run when the CustomSource is about to stop if your
      *     `start` code ran the last time CustomSource started
      */
