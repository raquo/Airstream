...
/** Flatten strategies with semantics of mirroring the latest emitted
  * observable.
  */
...
object FlattenStrategy {
  ...
  @deprecated(
    "You are using Airstream's deprecated flatMap operator using FlattenStrategy.flatMapAllowed import. This migration helper will be removed in the next version of Airstream. See https://github.com/raquo/Airstream/#avoid-unnecessary-flatmap",
    since = "17.0.0"
  )
  implicit lazy val allowFlatMap: AllowFlatMap = new AllowFlatMap {}

  @deprecated(
    "You are using Airstream's deprecated flatten operator using FlattenStrategy.flattenAllowed import. This migration helper will be removed in the next version of Airstream. See https://github.com/raquo/Airstream/#avoid-unnecessary-flatmap",
    since = "17.0.0"
  )
  implicit lazy val allowFlatten: AllowFlatten = new AllowFlatten {}

  /** See docs for [[SwitchStream]] */
  object SwitchStreamStrategy
      extends SwitchingStrategy[Observable, EventStream, EventStream] {
    override def flatten[A](
        parent: Observable[EventStream[A]]
    ): EventStream[A] = {
      new SwitchStream[EventStream[A], A](
        parent = parent,
        makeStream = identity
      )
    }
  ...
  object ConcurrentStreamStrategy
      extends MergingStrategy[Observable, EventStream, EventStream] {
    override def flatten[A](
        parent: Observable[EventStream[A]]
    ): EventStream[A] = {
  ...
  object SwitchSignalStreamStrategy
      extends SwitchingStrategy[EventStream, Signal, EventStream] {
  ...
  object SwitchSignalStrategy
      extends SwitchingStrategy[Signal, Signal, Signal] {
    override def flatten[A](parent: Signal[Signal[A]]): Signal[A] = {
      new SwitchSignal(parent)
    }
  }

  object SwitchSignalObservableStrategy
      extends SwitchingStrategy[Observable, Signal, Observable] {
