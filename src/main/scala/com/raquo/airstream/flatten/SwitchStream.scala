...
/** `parent` observable emits values that we convert into streams using
  * `makeStream`.
  *
  * This stream emits the events from the last such stream created this way.
  *
  * Events are emitted at the same time as the currently tracked stream emits
  * them (but in a new transaction).
  *
  * When `parent` emits a nextValue, this stream switches to emitting events
  * from `makeStream(nextValue)` (which is a stream).
  *
  * If parent stream emits an error, this stream re-emits that error and
  * unsubscribes from the last emitted stream
  *
  * If the stream created with makeStream emits an error, this stream re-emits
  * it in a new transaction.
  *
  * If parent is a signal in a failed state when SwitchEventStream is created,
  * parent's error is re-emitted in a new transaction, as if makeStream returned
  * a stream that emitted this error.
  *
  * Warning: Similar to [[com.raquo.airstream.eventbus.EventBus]], this stream
  * emits events in a new transaction because its proper topoRank would need to
  * be dynamic, which we don't support.
  *
  * @param makeStream
  *   Note: Must not throw
  */
class SwitchStream[I, O](
    ...
    with InternalNextErrorObserver[I] {

  override protected val topoRank: Int = 1

  private[this] val parentIsSignal: Boolean = parent.isInstanceOf[Signal[_]]

  private[this] var maybeCurrentEventStreamTry
      : js.UndefOr[Try[EventStream[O]]] = js.undefined

  private[this] var maybeNextEventStreamTry: js.UndefOr[Try[EventStream[O]]] =
    js.undefined

  // @TODO[Elegance] Maybe we should abstract away this kind of internal observer
  private[this] val internalEventObserver: InternalObserver[O] =
    InternalObserver[O](
      onNext = (nextEvent, _) => {
  ...
  override protected[this] def onStart(): Unit = {
    ...
    if (parentIsSignal) {
      ...
      maybeCurrentEventStreamTry.foreach {
        _.foreach { currentStream =>
          currentStream.addInternalObserver(
  ...
  private def switchToNextStream(
      nextStream: EventStream[O],
      isStarting: Boolean
  ): Unit = {
    ...
    if (!isSameStream || isStarting) {
      nextStream.addInternalObserver(
        internalEventObserver,
        shouldCallMaybeWillStart = !isStarting
      )
    }
  }

  private def switchToNextError(
      nextError: Throwable,
      transaction: Option[Transaction]
  ): Unit = {
    removeInternalObserverFromCurrentEventStream()
    maybeCurrentEventStreamTry = Failure(nextError)
    transaction.fold[Unit](Transaction(fireError(nextError, _)))(
      fireError(nextError, _)
