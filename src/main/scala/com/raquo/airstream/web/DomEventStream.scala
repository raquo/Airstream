...
object DomEventStream {
  ...
  /** This stream, when started, registers an event listener on a specific
    * target like a DOM element, document, or window, and re-emits all events
    * sent to the listener.
    *
    * When this stream is stopped, the listener is removed.
    *
    * @tparam Ev
    *   \- You need to specify what event type you're expecting. The event type
    *   depends on the event, i.e. eventKey. Look it up on MDN.
    *
    * @param eventTarget
    ...
    * @param useCapture
    *   See section about "useCapture" in
  ...
  def apply[Ev <: dom.Event](
    ...
    new CustomStreamSource[Ev]((fireValue, _, _, _) => {
