...
/** This implicit class provides debug* methods for observers.
  *
  * Every debug* method creates a new observer that performs the specified
  * behaviour and then calls the original observer.
  *
  * Note: type inference doesn't work on Observer debug* methods, so you need to
  * provide the AA type param (it should be same as A).
  */
class DebuggableObserver[A](val observer: Observer[A]) extends AnyVal {

  /** Create a new observer with a displayName, that sends all events to the
    * original observer. This is different from `setDisplayName`.
    *
    ...
    * `observer.setDisplayName("foo").debugLog()`, the logger's displayName will
    * be "foo|Debug" – with a suffix – to differentiate it from the "foo"
    * displayName of `observer` itself.
    */
  ...
  def debugSpy(fn: Try[A] => Unit): Observer[A] =
    new DebuggerObserver[A](observer, fn)
  ...
  /** Log emitted events and errors if `when` condition passes, using
    * dom.console.log if `useJsLogger` is true.
    */
  def debugLog(
