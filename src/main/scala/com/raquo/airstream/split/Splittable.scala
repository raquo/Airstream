...
/** The `split` operator needs an implicit instance of Splittable[M] in order to
  * work on observables of M[_]
  */
trait Splittable[M[_]] {
  ...
  /** Find the FIRST item matching predicate, and return a collection with it
    * updated
    */
...
object Splittable extends LowPrioritySplittableImplicits {
  ...
  implicit object ListSplittable extends Splittable[List] {
    ...
    override def map[A, B](inputs: List[A], project: A => B): List[B] =
      inputs.map(project)

    override def foreach[A](inputs: List[A], f: A => Unit): Unit =
      inputs.foreach(f)
  ...
  implicit object VectorSplittable extends Splittable[Vector] {
    ...
    override def map[A, B](inputs: Vector[A], project: A => B): Vector[B] =
      inputs.map(project)

    override def foreach[A](inputs: Vector[A], f: A => Unit): Unit =
      inputs.foreach(f)
  ...
  implicit object SetSplittable extends Splittable[Set] {
    ...
    override def map[A, B](inputs: Set[A], project: A => B): Set[B] =
      inputs.map(project)

    override def foreach[A](inputs: Set[A], f: A => Unit): Unit =
      inputs.foreach(f)
  ...
  implicit object OptionSplittable extends Splittable[Option] {
    ...
    override def map[A, B](inputs: Option[A], project: A => B): Option[B] =
      inputs.map(project)

    override def foreach[A](inputs: Option[A], f: A => Unit): Unit =
      inputs.foreach(f)
  ...
  implicit object BufferSplittable extends Splittable[mutable.Buffer] {
    ...
    override def map[A, B](
        inputs: mutable.Buffer[A],
        project: A => B
    ): mutable.Buffer[B] = inputs.map(project)

    override def foreach[A](inputs: mutable.Buffer[A], f: A => Unit): Unit =
      inputs.foreach(f)
  ...
  implicit object JsArraySplittable extends Splittable[JsArray] {
    ...
    override def map[A, B](inputs: JsArray[A], project: A => B): JsArray[B] =
      inputs.map(project)

    override def foreach[A](inputs: JsArray[A], f: A => Unit): Unit =
      inputs.forEach(f)
  ...
  implicit object JsVectorSplittable extends Splittable[JsVector] {
    ...
    override def map[A, B](inputs: JsVector[A], project: A => B): JsVector[B] =
      inputs.map(project)

    override def foreach[A](inputs: JsVector[A], f: A => Unit): Unit =
      inputs.forEach(f)
  ...
  implicit object ScalaJsArraySplittable extends Splittable[js.Array] {
    ...
    override def map[A, B](inputs: js.Array[A], project: A => B): js.Array[B] =
      inputs.map(project)

    override def foreach[A](inputs: js.Array[A], f: A => Unit): Unit =
      inputs.foreach(f)
...
/** If changing the order, check it first on all Scala versions (and be
  * disappointed). Some of the errors caused by implicit ambiguity are not very
  * deterministic in Scala 3. Be conservative.
  */
