...
trait EventStream[+A] extends Observable[A] {
  ...
  protected[this] val internal: EventStreamObservableInternal = new EventStreamObservableInternal()


  private[airstream] class EventStreamObservableInternal(
  ) extends ObservableInternal {

    override final def fireValue(nextValue: Any, transaction: Transaction): Unit = {
      ...
      internal.externalObservers.foreach { observer =>
        try {
      ...
      internal.internalObservers.foreach { observer =>
        observer.onNext(nextValue, transaction)
      }
    }

    override final def fireError(nextError: Throwable, transaction: Transaction): Unit = {
      // Note: Removal of observers is always done at the end of a transaction, so the iteration here is safe
    ...
    override final def fireTry(nextValue: Try[Any], transaction: Transaction): Unit = {
      nextValue.fold(
