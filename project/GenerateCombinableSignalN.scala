...
case class GenerateCombinableSignalN(
  sourceDir: File,
  from: Int,
  to: Int
) extends SourceGenerator(
  sourceDir / "scala" / "com" / "raquo" / "airstream" / "combine" / "generated" / s"CombinableSignalN.scala"
) {
  ...
  override def apply(): Unit = {
    ...
    line()
    line("// #Warning do not edit this file directly, it is generated by GenerateCombinableSignalN.scala")
    line()
    line("// Low-priority implicit: see CombinableSignal.scala for high-priority arities")
    line()
    enter("class CombinableSignalN[A](private val signal: Signal[A]) extends AnyVal {", "}") {
      line()
