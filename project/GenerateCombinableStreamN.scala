...
case class GenerateCombinableStreamN(
  sourceDir: File,
  from: Int,
  to: Int
) extends SourceGenerator(
  sourceDir / "scala" / "com" / "raquo" / "airstream" / "combine" / "generated" / s"CombinableStreamN.scala"
) {
  ...
  override def apply(): Unit = {
    ...
    line()
    line("// #Warning do not edit this file directly, it is generated by GenerateCombinableStreamN.scala")
    line()
    line("// Low-priority implicit: see CombinableStream.scala for high-priority arities")
    line()
    enter(s"class CombinableStreamN[A](private val stream: EventStream[A]) extends AnyVal {", "}") {
      line()
